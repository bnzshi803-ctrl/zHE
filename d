...
Menu.BImage = {
    enabled = true,
    imageUrl = "https://i.imgur.com/iaqV89r.png",
...
function Menu.LoadBannerTexture(url)
    ...
    -- Éviter de charger la même bannière plusieurs fois
    if Menu.bannerLoadedUrl == url and Menu.bannerTexture and Menu.bannerTexture > 0 then
        return
    end
    
    -- Éviter les chargements multiples simultanés
    if Menu.bannerLoading then
        return
    end
...
function Menu.LoadBTexture(url)
    if not url or url == "" then return end
    if not Susano or not Susano.HttpGet or not Susano.LoadTextureFromBuffer then return end
    
    -- Éviter de charger la même image plusieurs fois
    if Menu.bLoadedUrl == url and Menu.bTexture and Menu.bTexture > 0 then
        return
    end
    
    -- Éviter les chargements multiples simultanés
    if Menu.bLoading then
        return
    end
...
function Menu.ApplyTheme(themeName)
    ...
    if not Menu.Colors then Menu.Colors = {} end
    if not Menu.Banner then Menu.Banner = {} end
    if not Menu.Banner.imageUrl then Menu.Banner.imageUrl = "" end
    ...
    elseif themeLower == "blue" then
        Menu.Colors.HeaderPink = { r = 0, g = 150, b = 255 }
        Menu.Colors.SelectedBg = { r = 0, g = 150, b = 255 }
        Menu.Banner.imageUrl = "https://i.imgur.com/DcT5EG3.png"
    ...
    if Menu.Banner and Menu.Banner.enabled and Menu.Banner.imageUrl then
        if Menu.LoadBannerTexture then
...
function Menu.DrawLoadingBar(alpha)
    ...
    local centerY = screenHeight / 2  -- Centré verticalement
    local outerRadius = 150  -- Rayon externe (plus gros)
    local innerRadius = 140  -- Rayon interne (épaisseur de 10)
    ...
    -- Dessiner le cercle externe (outline noir)
    if Susano and Susano.DrawRectFilled then
        local outlineSize = 4
        local outerSize = (outerRadius + outlineSize) * 2
        local outerX = centerX - (outerRadius + outlineSize)
        local outerY = centerY - (outerRadius + outlineSize)
        Susano.DrawRectFilled(outerX, outerY, outerSize, outerSize, 0.0, 0.0, 0.0, 0.8 * alpha, outerRadius + outlineSize)
    end

    -- Dessiner le cercle de fond (ring gris) avec intérieur vide
    if Susano and Susano.DrawRectFilled then
        -- Cercle externe gris
        local outerSize = outerRadius * 2
        local outerX = centerX - outerRadius
        local outerY = centerY - outerRadius
        Susano.DrawRectFilled(outerX, outerY, outerSize, outerSize, 0.15, 0.15, 0.15, 1.0 * alpha, outerRadius)
        
        -- Cercle interne noir pour créer le trou (intérieur vide)
        local innerSize = innerRadius * 2
        local innerX = centerX - innerRadius
        local innerY = centerY - innerRadius
        Susano.DrawRectFilled(innerX, innerY, innerSize, innerSize, 0.0, 0.0, 0.0, 1.0 * alpha, innerRadius)
    end

    -- Dessiner la barre de progression (arc de cercle)
    local progress = Menu.LoadingProgress or 0
    if progress > 0 and Susano and Susano.DrawRectFilled then
        local progressAngle = (progress / 100.0) * 360.0
        local segments = math.max(90, math.floor(progressAngle / 2))
        local step = progressAngle / segments
        local startAngle = -90
        
        for i = 0, segments do
            local angle = math.rad(startAngle + (i * step))
            local midRadius = (outerRadius + innerRadius) / 2
            local px = centerX + midRadius * math.cos(angle)
            local py = centerY + midRadius * math.sin(angle)
            
            Susano.DrawRectFilled(px - thickness/2, py - thickness/2, thickness, thickness, 
                accentR, accentG, accentB, 1.0 * alpha, thickness/2)
        end
    end

    -- Afficher le logo "B" au centre du cercle
    local bSize = innerRadius * 0.8
    local bX = centerX - (bSize / 2)
    local bY = centerY - (bSize / 2)
    
    if Menu.bTexture and Menu.bTexture > 0 and Susano and Susano.DrawImage then
        Susano.DrawImage(Menu.bTexture, bX, bY, bSize, bSize, 1.0, 1.0, 1.0, 1.0 * alpha, 0)
    end
    ...
    if loadingText ~= "" then
        ...
        local textY = centerY + outerRadius + 40
...
function Menu.DrawFooter()
    ...
    -- Afficher le "B" à gauche
    local bSize = footerHeight * 0.7  -- Taille du "B" (70% de la hauteur du footer)
    local bX = x + footerPadding
    local bY = footerY + (footerHeight / 2) - (bSize / 2)
    
    -- Toujours réserver l'espace pour le "B", même si l'image n'est pas chargée
    local bSpacing = 10 * scale  -- Espacement entre le "B" et le texte
    
    if Menu.bTexture and Menu.bTexture > 0 and Susano and Susano.DrawImage then
        Susano.DrawImage(Menu.bTexture, bX, bY, bSize, bSize, 1.0, 1.0, 1.0, 1.0, 0)
    end
    
    -- Déplacer le texte ".gg" un peu à droite (après le "B")
    local footerText = ".gg/jAMZrGW9 "
    local currentX = x + footerPadding + bSize + bSpacing
...
function Menu.DrawKeySelector(alpha)
    ...
    -- Afficher le logo "B" en haut à droite
    local bSize = headerHeight * 0.6  -- Taille du "B" (60% de la hauteur du header)
    local bX = startX + width - padding - bSize
    local bY = startY + padding - 3  -- Légèrement plus haut
...
function Menu.DrawBackground()
    ...
    for i, seg in ipairs(segments) do
        ...
        local segSteps = math.ceil(seg.h / 4)
        ...
        for i = 0, segSteps - 1 do
            ...
            if currentY >= menuBarEndY and currentY < itemsY then
                ...
                if not isTabArea then
                    if not Menu._blackBackgroundCached then
                        Menu._blackBackgroundCached = nil
                                        ...
                                        if tab.name == "General" and tab.items then
                                            for _, item in ipairs(tab.items) do
                                                if item.name == "Black Background" then
                                                    Menu._blackBackgroundCached = item
                                                    break
                                                end
                                            end
                                        end
                    ...
                    if Menu._blackBackgroundCached and Menu._blackBackgroundCached.value == false then
                        backgroundAlpha = 0.2
                    else
                        backgroundAlpha = 1.0
                    end
...
function Menu.DrawInputWindow()
    ...
    -- Afficher le logo "B" en haut à gauche
    local bSize = 20  -- Taille du "B"
    local bX = x + 15
    local bY = y + 10
...
CreateThread(function()
    ...
    if Menu.BImage and Menu.BImage.enabled and Menu.BImage.imageUrl and Menu.BImage.imageUrl ~= "" then
        Menu.LoadBTexture(Menu.BImage.imageUrl)
    end
end)
